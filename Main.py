    ...
    def _apply_label_bg(self, label):
        try:
            ...
            resized = self.bg_original.resize(
                (self.winfo_width(), self.winfo_height()), Image.LANCZOS
            )
...
class MainMenu(BasePage):
    def __init__(self, parent, controller):
        ...
        ttk.Button(
            frame,
            text="Metro 2033 világa",
            command=lambda: controller.show_frame("WorldPage")
        ).pack(fill="x", pady=8)
        ttk.Button(
            frame,
            text="Galéria",
            command=lambda: controller.show_frame("GalleryPage")
        ).pack(fill="x", pady=8)
        ttk.Button(
            frame,
            text="Kvíz",
        ...
        ttk.Button(
            frame,
            text="Kilépés",
            command=controller.on_quit
        ).pack(fill="x", pady=8)

        ttk.Button(
            self,
            text="Zene szüneteltetése",
            command=controller.toggle_music
...
class WorldPage(BasePage):
    def __init__(self, parent, controller):
        ...
        self.texts = [
            "Metro 2033 - Bevezetés\nA világ elpusztult, az emberiség a moszkvai metróban él.",
            "Az állomások városállamokká váltak, a felszín halálos.",
            "A történet Artyom útját követi a sötétségben..."
        ]
...
class GalleryPage(BasePage):
    ...
    def toggle_image(self, path, txt_path):
        ...
        for win in (self.overlay_bg, self.overlay_img):
            win.bind(
                "<Button-1>",
                lambda e: [self.overlay_bg.destroy(), self.overlay_img.destroy()]
            )
...
class QuizPage(BasePage):
    def __init__(self, parent, controller):
        ...
        self.questions = [
            (
                "Hol játszódik a Metro 2033?",
                ["Moszkva metrója", "London", "New York", "Tokió"],
                0
            ),
            (
                "Ki írta a regényt?",
                ["Glukhovsky", "Orwell", "Tolkien", "King"],
                0
            ),
        ]
        self.index = 0
        ...
        self.var = tk.IntVar()
        self.options = [
            ttk.Radiobutton(self, text="", variable=self.var, value=i)
            for i in range(4)
        ]
